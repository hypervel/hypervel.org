...
If you would like to retrieve a single row from a database table, but throw an `Hyperf\Database\Model\ModelNotFoundException` if no matching row is found, you may use the `firstOrFail` method. If the `ModelNotFoundException` is not caught, a 404 HTTP response is automatically sent back to the client:
...
You may use the `toSql` and `toRawSql` methods while building a query to dump the current query bindings and SQL. The `toSql` method will display the compiled sql `without` binding parameters. The `toRawSql` method will display the compiled sql `with` binding parameters:

```php
DB::table('users')->where('votes', '>', 100)->toSql();

DB::table('users')->where('votes', '>', 100)->toRawSql();
```