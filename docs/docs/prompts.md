...
![Prompts](/images/prompts-example.png)

Hypervel Prompts is perfect for accepting user input in your [Artisan console commands](/docs/artisan#writing-commands), but it may also be used in any command-line PHP project.
...
Alternatively, you may leverage the power of Hypervel's [validator](/docs/validation). To do so, provide an array containing the name of the attribute and the desired validation rules to the `validate` argument:
...
public function testReportGeneration(): void
{
    $this->artisan('report:generate')
        ...
        ->expectsPrompexpectsPromptsWarningtsTable(
            headers: ['Name', 'Email'],
            rows: [
